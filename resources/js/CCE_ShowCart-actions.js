...
function CCE_prepPevents() {
    ...
    elems = document.getElementsByName('CCE_btnList');
    for ( const elem of elems) {
...
function clickCA_delete(trElem, celem, celemt, delem) {
    ...
    for ( const tbody of tbodies) {
        let tbadge = tbody.parentNode.querySelector('table > thead > tr > th > div > span');  // we need the element that hosts the actual counter ...
...
function clickCCEtoList(belem) {
    //         button div        div-action
    let xelem = belem.parentNode.parentNode;

    let XREFs = [];
    //                th         tr         thead      tbody
    let tbody = xelem.parentNode.parentNode.parentNode.nextElementSibling;

    collectXREF(tbody, XREFs);

    if (XREFs.length > 0) {
        execCCEtoList(xelem, XREFs);
    }
}
function execCCEtoList(xelem, XREFs) {
    let route_ajax = xelem.getAttribute('action');
    let formData = new FormData();
    formData.append('xrefs', XREFs.join(';'));
    exec_wtHTTPpostDummy(route_ajax, formData); // , 'click_CCE_btnLIST');
}
...
function clickCSVimport() {
    ...
    if (files.length == 1) {
        ...
        if (files.length > 1) {
            msg = document.getElementById('mult_files');
        }
        msg.classList.toggle('is-hidden');
    }
}
...
function exec_wtHTTPpostDummy(url, FData, callback=null) {
    // const modal = document.getElementById('wt-ajax-modal')
    // const modal_content = modal.querySelector('.modal-content');
    // const select = document.getElementById(modal_content.dataset.wtSelectId);
...
function click_CCE_btnLIST(hideElem) {
    // close the modal
    if (hideElem) {
        let nElem     = document.querySelectorAll(hideElem)[0];
        nElem.click();
    }
    // open the list view
    let btnEXPORT = document.getElementById('CCE_menuINDIlist');
    btnEXPORT.click();
}
